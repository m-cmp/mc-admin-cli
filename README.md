...
Before running `installAll.sh`, decide which mode fits your environment and set `MC_IAM_MANAGER_PUBLIC_DOMAIN` in `conf/docker/conf/mc-iam-manager/.env` (or `conf/docker/.env.setup` when using the setup workflow):

| | **Mode A — Local / Dev** | **Mode B — Production** |
|---|---|---|
| Domain | Any local name (e.g. `mciam.local`) | Public FQDN (e.g. `mciam.example.com`) |
| TLS Certificate | Self-signed (generated by `0_preset_dev.sh`) | Let's Encrypt via certbot (`0_preset_prod.sh`) |
| DNS | Add to `/etc/hosts` manually | Public DNS A-record → server IP |
| installAll.sh flag | `--mode dev` | `--mode prod` |
| Browser warning | Certificate warning (acceptable for dev) | No warning (trusted CA) |

**Mode A — edit `MC_IAM_MANAGER_PUBLIC_DOMAIN` to a local domain:**
```shell
# conf/docker/conf/mc-iam-manager/.env
MC_IAM_MANAGER_PUBLIC_DOMAIN=mciam.local    # or any hostname you prefer

# Add to /etc/hosts on the client machine
echo "127.0.0.1  mciam.local" | sudo tee -a /etc/hosts
```

**Mode B — set your registered FQDN (DNS A-record must point to this server's public IP):**
```shell
# conf/docker/conf/mc-iam-manager/.env
MC_IAM_MANAGER_PUBLIC_DOMAIN=mciam.example.com

# Issue a Let's Encrypt certificate before starting (port 80 must be free)
docker compose -f conf/docker/docker-compose.cert.yaml --env-file conf/docker/.env up
```

## Step 3. Run installAll.sh

`installAll.sh` generates TLS certificates (Mode A) or validates them (Mode B), creates the nginx configuration from the template, and optionally starts all containers.

```shell
# Interactive mode
./installAll.sh

# Non-interactive: Mode A — generate self-signed cert and start in background
./installAll.sh --mode dev --run background

# Non-interactive: Mode B — validate cert and start in background
./installAll.sh --mode prod --run background

# Non-interactive: generate config files only, skip container start
