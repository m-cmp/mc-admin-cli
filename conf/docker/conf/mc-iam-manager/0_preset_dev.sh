...
echo "Checking ${DOMAIN_NAME} in ${HOSTS_FILE}..."

# 더 정확한 패턴 매칭을 위한 정규식 사용
# 127.0.0.1 도메인명 형태의 라인이 있는지 확인 (공백/탭 문자 고려)
if grep -E "^[[:space:]]*127\.0\.0\.1[[:space:]]+${DOMAIN_NAME}[[:space:]]*$" "${HOSTS_FILE}" > /dev/null; then
    echo "✓ ${DOMAIN_NAME} already exists in ${HOSTS_FILE}. Skipping."
else
    # 기존에 다른 형태로 추가된 항목이 있는지 확인하고 제거
    echo "Removing any existing entries for ${DOMAIN_NAME}..."
    sudo sed -i "/[[:space:]]*127\.0\.0\.1[[:space:]]\+${DOMAIN_NAME}[[:space:]]*$/d" "${HOSTS_FILE}"
    
    # hosts 파일에 추가 (sudo 권한 필요)
    echo "Adding 127.0.0.1 ${DOMAIN_NAME} to ${HOSTS_FILE}..."
